...
def generate_radius(density, max_fiber, min_fiber, seed):
    np.random.seed(seed)
    # 預先生成2000個隨機半徑
    area_limit = (density / 100) *0.01 ## density unit %
    ...
    # 找到第一個使得累積面積超過面積限制的索引
    index = np.where(cumulative_area >= area_limit)[0][0]
    
    # 保留在索引之前（不包括觸發限制的那一個）的所有半徑
...
particle_radius = generate_radius(density,0.1/40,0.1/60,seed1)
