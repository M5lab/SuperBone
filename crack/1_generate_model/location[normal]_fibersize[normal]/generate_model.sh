...
python3 0_generate_fiber_location.py --seed1 "$density" --seed2 "$fiber_seed" --seed3 "$loc_seed"

# Combine and trim step
echo "Running 1_combine&trim.py..."
python3 2_combine\&trim.py --seed2 "$fiber_seed" --seed3 "$loc_seed"

# 執行最終模型生成
echo "Preparing and running 2_generate_final_model.in..."
lmp -var SEED0 "$density" -var SEED1 "$fiber_seed" -var SEED2 "$loc_seed" -in 3_generate_final_model.in 
