...
def parse_args():
    ...
    parser.add_argument('-infile', metavar='images', type=str,
                        default='./resize.png',help='absolute or relative address of image files')
    parser.add_argument('-o', dest='outfile', metavar='data file', type=str, default='nobond_model_src')
    parser.add_argument('-lx', dest='lx', metavar='Lx', type=float, default=0.00816,
                        help='model size in x dimension\t(default = 0.00816)')
    parser.add_argument('-ly', dest='ly', metavar='Ly',type=float, default=0.00816,
                        help='model size in y dimension\t(default = 0.00816)')
    parser.add_argument('-lz', dest='lz', metavar='Lz',type=float, default=1,
                        help='model size in z dimension\t(default = 1)')
    parser.add_argument('-s', dest='s', metavar='lc', type=float, default=0.00003657,
                        help='lattice constant = particle spacing along (0.00003657)\t(default = 0.00003657)')
    parser.add_argument('-t', dest='types', metavar='types', type=int, default=2,
                        help='number of particle types')
    parser.add_argument('-n',dest='nsize', metavar='len', nargs=2, type=float, default=[7.968e-5, 1.083e-3],
                        help='create notch with lengths in x and y dimension\t(default = [7.968e-5, 1.083e-3])')
...
if args.outfile is None:
    outfile = 'nobond_model_src/'+os.path.splitext(infile)[0] + '.data'
else:
...
with open(outfile, "w") as outfile:
    ...
    outfile.write("\nAtoms # bond\n\n")
    for i in range(natoms):
            outfile.write("%5d\t%d\t%d\t%10.8f\t%10.8f\t%10.8f\t%d\t%d\t%d\n" % (atoms[i,dtype.get('id')],
                                                                          atoms[i,dtype.get('molecule')],
                                                                          atoms[i,dtype.get('type')],
                                                                          atoms[i,dtype.get('x')],
                                                                          atoms[i,dtype.get('y')],
