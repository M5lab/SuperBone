...
class ImgtoParticle_si:

    def __init__(self, img_path, resize_img, output_dir, std_pixel_spacing, simulation):
        self.img_path = img_path
        self.resize_img = resize_img
        self.output_dir = output_dir
        self.std_pixel_spacing = std_pixel_spacing
        self.simulation = simulation
        
    def read_data(self):
        global lx, ly, lz, s, atom_types, nsize
        lx, ly, lz = self.resize_img.shape[0]*(self.std_pixel_spacing/1000), self.resize_img.shape[0]*(self.std_pixel_spacing/1000) ,1
        s = 0.00016
        # s = 100
        atom_types = 2
        nsize = [lx*(250/(256*100)), ly*(3400/(256*100))]       
    ...
    def run(self):
        ...
        plt.close()
        '''
        outfile = infile + '_si_{}.data'.format(self.simulation)
        ...
        with open(outfile, "w") as outfile:
            ...
            outfile.write("\nAtoms # bond\n\n")
            for i in range(natoms):
                    outfile.write("%5d\t%d\t%d\t%10.6f\t%10.6f\t%10.6f\t%d\t%d\t%d\n" % (atoms[i,dtype.get('id')],
                                                                                  atoms[i,dtype.get('molecule')],
                                                                                  atoms[i,dtype.get('type')],
                                                                                  atoms[i,dtype.get('x')],
                                                                                  atoms[i,dtype.get('y')],
