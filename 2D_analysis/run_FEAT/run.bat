...
rem check_or_not
if ["%CHECK%"] == ["FALSE"] goto :check_img_src
...
:check_img_src
dir "%cd%\img_src" >temp\stdout.txt 2>temp\stderr.txt
if %ERRORLEVEL% == 0 goto :launch_python
echo Couldn't find directory "img_src" in %cd%
goto :show_error

:launch_python
for %%i in (%cd%\img_src\*) do (
	rem record current processing image
	set img_name=%%~nxi
	echo !img_name! >>temp\%record_txt%
	
	rem launch python
	set rootname=%%~ni
	%PYTHON% launch.py -i img_src\!img_name!
)
